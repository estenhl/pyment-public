...
There are two approaches for running the models locally, via Docker or by installing the Python-package locally. Running the model with Docker requires less setup, but is less flexible.
...
Both inference and finetuning rely on a GPU-based TensorFlow parser for MGH/MGZ images from a [separate repository](https://github.com/estenhl/tensorflow-neuroimaging). Given its developmental status, it is worth sanity checking this loader on the exact images you intend to use before proceeding. First generate the FastSurfer crops:
```
python scripts/create_fastsurfer_conformed_crops.py \
    $HOME/data/ds000030/outputs/fastsurfer/ \
...
```
pyment-verify-loader $HOME/data/ds000030/outputs/fastsurfer -r crop.mgz
```
