...
def create_fastsurfer_conformed_crops(
    root: str, size: int, num_threads: int
) -> None:
    ...
    ensure_fastsurfer_crops_exists(folders, num_threads=num_threads)
...
if __name__ == '__main__':
    ...
    parser.add_argument('root', help='Path to FastSurfer output directory')
    parser.add_argument(
        '-n',
        '--size',
        default=None,
        help='Number of images from the given folder to process',
    )
    parser.add_argument(
        '-nt',
        '--num_threads',
    ...
    create_fastsurfer_conformed_crops(
        args.root, size=args.size, num_threads=args.num_threads
    )
