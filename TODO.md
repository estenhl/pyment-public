...
- [ ] **Replace IXI with ds000030 (OpenNeuro)** — the IXI download URL (biomedic.doc.ic.ac.uk) now returns 403. Replacement: [ds000030](https://openneuro.org/datasets/ds000030) (UCLA Consortium for Neuropsychiatric Phenomics, 272 subjects: healthy controls + schizophrenia/bipolar/ADHD). `participants.tsv` provides `age` (continuous) and `diagnosis` (4-class, collapsible to binary) — suitable targets for tutorial finetuning studies.
  - [ ] Replace `scripts/download_ixi.py` and `scripts/create_ixi_labels.py` with a single `scripts/download_dataset.py` that downloads a configurable number of ds000030 subjects via the OpenNeuro S3/GitHub raw URL and writes a `labels.csv` with `participant_id`, `age`, and `diagnosis` columns
  - [ ] Rewrite `scripts/evaluate_ixi_predictions.py` for ds000030 — update subject-ID parsing (BIDS `sub-XXXXX` format) and metadata join to use `participants.tsv` columns
  ...
  - [ ] Update `README.md` — replace all ~15 IXI references across the download, preprocess, predict, evaluate, and finetune tutorial sections
