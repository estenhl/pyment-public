...
def test_json_serialize_numpy_scalars_and_arrays():
    ...
    assert serialized == {
        ...
        'Expected json_serialize to return native Python types for numpy '
        'scalars and arrays'
    )
    json.dumps(serialized, allow_nan=False)
...
def test_json_serialize_nested_lists_and_dicts():
    ...
    assert serialized == {
        ...
        'Expected json_serialize to return correctly serialized nested '
        'structures'
    )
    json.dumps(serialized, allow_nan=False)
...
def test_json_serialize_unknown_type_passthrough():
    ...
    assert serialized['items'] is payload['items'], (
        'Expected json_serialize to return unknown types unchanged'
    )
