...
def test_has_regression_weights_no_top():
    try:
        ...
        assert False, ('Unable to load pretrained RegressionSFCN for brain '
                       'age without top layers')

def test_has_soft_classification_brain_age_weights():
    try:
        SoftClassificationSFCN(weights='brain-age')
    except Exception:
        assert False, ('Unable to load pretrained SoftClassificationSFCN for '
                       'brain age')
